...
const snake = {
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up': 
        point = {x: headPoint.x, y: headPoint.y - 1};
        break;
      case 'right': 
        point = {x: headPoint.x + 1, y: headPoint.y};
        break;
      case 'down': 
        point = {x: headPoint.x, y: headPoint.y + 1};
        break;
    ...
    if(point.x == settings.colsCount) {
      point.x = 0;
    } else if(point.x == -1) {
      point.x = settings.colsCount - 1;
    }

    if(point.y == settings.rowsCount) {
      point.y = 0;
    } else if(point.y == -1) {
      point.y = settings.rowsCount - 1;
    }
...
const count = {
  ...
  render() {
    document.getElementById('count-value').innerHTML = this.value; 
  }
}
...
const game = {
  ...
  canMakeStep() {
    ...
    return !this.snake.isOnPoint(nextStepPoint) &&
        ...
        // nextStepPoint.x >= 0 &&
        // nextStepPoint.y >= 0 && 
        !this.barrier.isOnPoint(nextStepPoint);
  },
